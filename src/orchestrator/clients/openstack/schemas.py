...
class ServerConfig(BaseModel):
    ...
    networks: list[str] = Field(
        default_factory=list, description="List of network IDs to attach"
    )
    ...
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Server metadata"
    )
...
class ServerStatus(BaseModel):
    ...
    addresses: dict[str, Any] = Field(
        default_factory=dict, description="Network addresses"
    )
...
class SubnetConfig(BaseModel):
    ...
    dns_nameservers: list[str] = Field(
        default_factory=list, description="DNS nameserver IPs"
    )
    allocation_pools: list[dict[str, str]] = Field(
        default_factory=list,
        description="IP allocation pools [{'start': '...', 'end': '...'}]",
    )
    host_routes: list[dict[str, str]] = Field(
        default_factory=list, description="Host routes"
    )
...
class TokenResponse(BaseModel):
    ...
    catalog: list[dict[str, Any]] = Field(
        default_factory=list, description="Service catalog"
    )
